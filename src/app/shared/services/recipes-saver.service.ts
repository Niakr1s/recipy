import { Injectable } from '@angular/core';
import { deserializeArray, serialize } from 'class-transformer';
import { Ingridient } from 'src/app/models/ingridient.model';
import { Recipe } from 'src/app/models/recipe.model';

export const RecipesSaverServiceInjectKey = 'RecipesSaverService';

export interface RecipesSaverService {
  saveRecipes(recipes: Recipe[]): Promise<void>;
  loadRecipes(): Promise<Recipe[]>;
}

@Injectable()
export class RecipesSaverLocalStorageService implements RecipesSaverService {
  private static key = 'recipes';

  constructor() {}

  async saveRecipes(recipes: Recipe[]): Promise<void> {
    localStorage.setItem(
      RecipesSaverLocalStorageService.key,
      serialize(recipes)
    );
  }
  async loadRecipes(): Promise<Recipe[]> {
    const savedPlayRecipes = localStorage.getItem(
      RecipesSaverLocalStorageService.key
    );
    if (!savedPlayRecipes) {
      return [];
    }
    const recipes = deserializeArray(Recipe, savedPlayRecipes);
    Recipe.nextId = recipes.length;
    return recipes;
  }
}

@Injectable()
export class RecipesSaverMockStorageService implements RecipesSaverService {
  private initialRecipes = [
    Recipe.create({
      name: 'Tofu',
      detail: 'Tofu detail',
      imagePath:
        'https://cdn.loveandlemons.com/wp-content/uploads/2019/12/tofu-500x500.jpg',
      ingridients: [
        Ingridient.create({ name: 'soy', amount: 500, measurement: 'g' }),
        Ingridient.create({ name: 'water', amount: 1000, measurement: 'ml' }),
      ],
    }),
    Recipe.create({
      name: 'Pork',
      detail: 'Pork detail',
      imagePath:
        'https://images.immediate.co.uk/production/volatile/sites/30/2020/08/roast-rack-of-pork-with-wild-garlic-stuffing_1-536cd86.jpg?quality=90&resize=960,872',
      ingridients: [
        Ingridient.create({ name: 'meat', amount: 1, measurement: 'Kg' }),
        Ingridient.create({ name: 'oil', amount: 10, measurement: 'g' }),
      ],
    }),
    Recipe.create({
      name: 'Oolong tea',
      detail: 'Oolong tea detail',
      imagePath:
        'data:image/jpeg;base64,',
      ingridients: [
        Ingridient.create({ name: 'water', amount: 1, measurement: 'L' }),
        Ingridient.create({ name: 'tea', amount: 1, measurement: 'g' }),
      ],
    }),
  ];

  async saveRecipes(recipes: Recipe[]): Promise<void> {
    this.initialRecipes = [...recipes];
  }
  async loadRecipes(): Promise<Recipe[]> {
    return [...this.initialRecipes];
  }
}
