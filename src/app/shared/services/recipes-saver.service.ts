...
export class RecipesSaverLocalStorageService implements RecipesSaverService {
  ...
  async saveRecipes(recipes: Recipe[]): Promise<void> {
    localStorage.setItem(RecipesSaverLocalStorageService.key, serialize(recipes));
  }
  async loadRecipes(): Promise<Recipe[]> {
    const savedPlayRecipes = localStorage.getItem(RecipesSaverLocalStorageService.key);
    if (!savedPlayRecipes) { return []; }
    return deserializeArray(Recipe, savedPlayRecipes);
  }
}
